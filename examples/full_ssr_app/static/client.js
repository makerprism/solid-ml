...
function parse_sign_and_base(s) {
  ...
  if (s[i2] === "0") {
    const match$1 = s.charCodeAt(i2 + 1 | 0);
    if (match$1 >= 89) {
      if (match$1 >= 111) {
        if (match$1 < 121) {
          switch (match$1) {
...
function caml_int_of_string(s) {
  ...
  const len = s.length;
  const c = i2 < len ? s.charCodeAt(i2) : (
    /* '\000' */
    0
  );
  ...
  const res = match[1] * aux(d, i2 + 1 | 0);
  const or_res = res | 0;
  ...
  while (true) {
    ...
    if (c >= 69) {
      if (c >= 88) {
        ...
        f2.signedconv = true;
        f2.uppercase = true;
        f2.conv = String.fromCharCode(lowercase(c));
        _i = i2 + 1 | 0;
        continue;
      }
    } else {
      switch (c) {
        ...
        case 46:
          ...
          while ((function() {
            ...
            f2.prec = (Math.imul(f2.prec, 10) + fmt.charCodeAt(j) | 0) - /* '0' */
            48 | 0;
            j = j + 1 | 0;
          }
          ;
    ...
    switch (exit2) {
      case 1:
        ...
        continue;
      case 2:
        f2.signstyle = String.fromCharCode(c);
        _i = i2 + 1 | 0;
        continue;
      case 3:
        f2.width = 0;
        ...
        while ((function() {
          ...
          f2.width = (Math.imul(f2.width, 10) + fmt.charCodeAt(j$1) | 0) - /* '0' */
          48 | 0;
          j$1 = j$1 + 1 | 0;
        }
        ;
...
function caml_format_int(fmt, i2) {
  if (fmt === "%d") {
    return String(i2);
  }
  const f2 = parse_format(fmt);
  const i$1 = i2 < 0 ? f2.signedconv ? (f2.sign = -1, -i2 >>> 0) : i2 >>> 0 : i2;
  let s = i$1.toString(int_of_base(f2.base));
  if (f2.prec >= 0) {
    f2.filter = " ";
    const n = f2.prec - s.length | 0;
    if (n > 0) {
      s = "0".repeat(n) + s;
    }
  }
  return finish_formatting(f2, s);
}
...
function caml_int64_format(fmt, x) {
  ...
  const f2 = parse_format(fmt);
  const x$1 = f2.signedconv && i64_lt(x, zero) ? (f2.sign = -1, neg(x)) : x;
  const match = f2.base;
  let s;
  switch (match) {
    case /* Oct */
  ...
  return finish_formatting(f2, fill_s);
}
function caml_format_float(fmt, x) {
  const f2 = parse_format(fmt);
  const prec = f2.prec < 0 ? 6 : f2.prec;
  const x$1 = x < 0 ? (f2.sign = -1, -x) : x;
  let s = "";
  if (isNaN(x$1)) {
    s = "nan";
    f2.filter = " ";
    ...
    switch (match) {
      case "e":
        ...
        const i2 = s.length;
        if (s[i2 - 3 | 0] === "e") {
          s = s.slice(0, i2 - 1 | 0) + ("0" + s.slice(i2 - 1 | 0));
        }
        break;
      case "f":
        s = x$1.toFixed(prec);
...
function app(_f, _args) {
  while (true) {
    ...
    if (d >= 0) {
      return function(x) {
        return app(f2, args.concat([x]));
      };
    }
    _args = sub2(args, arity, -d | 0);
    _f = f2.apply(null, sub2(args, 0, arity));
    continue;
  }
  ;
}
...
// node_modules/melange.js/caml_external_polyfill.js
...
function caml_fill_bytes(s, i2, l, c) {
  if (l <= 0) {
    return;
  }
  for (let k = i2, k_finish = l + i2 | 0; k < k_finish; ++k) {
    s[k] = c;
  }
}
...
function caml_blit_string(s1, i1, s2, i2, len) {
  ...
  if (len <= off1) {
    for (let i3 = 0; i3 < len; ++i3) {
      s2[i2 + i3 | 0] = s1.charCodeAt(i1 + i3 | 0);
    }
    return;
  }
  for (let i$1 = 0; i$1 < off1; ++i$1) {
...
function query_selector_all_within(el, selector) {
  return to_list(el.querySelectorAll(selector));
}
...
function get_inner_text(prim) {
  return prim.innerText;
}
function query_selector_within(prim0, prim1) {
  return nullable_to_opt(prim0.querySelector(prim1));
}
...
function map_dps(_dst, _offset, f2, _param) {
  while (true) {
    const dst = _dst;
...
function map(f2, param) {
  ...
  if (match) {
    ...
    return {
      hd: r1,
      tl: (map_dps(block, "tl", f2, match.tl), block)
    };
  }
  const r1$1 = _1(f2, a1);
  return {
...
function filter_map(f2, _param) {
  while (true) {
    ...
    const l = param.tl;
    const v = _1(f2, param.hd);
    if (v !== void 0) {
      const block = {
        hd: valFromOption(v),
        tl: 24029
      };
      filter_map_dps(block, "tl", f2, l);
      return block;
    }
    _param = l;
    continue;
...
function filter_map_dps(_dst, _offset, f2, _param) {
  while (true) {
    const dst = _dst;
    const offset = _offset;
    const param = _param;
...
function unsafe_escape(s) {
  let n = 0;
  for (let i2 = 0, i_finish = s.length; i2 < i_finish; ++i2) {
    const match = s[i2];
...
function fix_padding(padty, width, str) {
  ...
  switch (padty$1) {
    ...
    2:
      ...
      45 || get2(str, 0) === /* ' ' */
      32)) {
        set2(res, 0, get2(str, 0));
        blit4(str, 1, res, (width$1 - len | 0) + 1 | 0, len - 1 | 0);
      } else if (len > 1 && get2(str, 0) === /* '0' */
      48 && (get2(str, 1) === /* 'x' */
      120 || get2(str, 1) === /* 'X' */
      88)) {
        set2(res, 1, get2(str, 1));
  ...
  if (c >= 58) {
    ...
    switch (c) {
      ...
      case 47:
        return str;
      case 48:
        if ((prec$1 + 2 | 0) > len && len > 1 && (get2(str, 1) === /* 'x' */
        120 || get2(str, 1) === /* 'X' */
        88)) {
          const res = make4(
            prec$1 + 2 | 0,
...
function transform_int_alt(iconv, s) {
  ...
  const put = function(c) {
    set2(buf, pos2.contents, c);
    pos2.contents = pos2.contents + 1 | 0;
  };
...
function make_custom(k, acc, rest, arity, f2) {
  if (
    /* tag */
  ...
  return function(x) {
    return make_custom(k, acc, rest, arity$1, _1(f2, x));
  };
}
...
function register_printer(fn) {
  while (true) {
    const old_printers = get4(printers);
    const new_printers = {
      hd: fn,
      tl: old_printers
    };
    const success = compare_and_set2(printers, old_printers, new_printers);
    if (success) {
      return;
    }
    continue;
...
register_printer(function(exn) {
  if (exn.MEL_EXN_ID === Finally_raised) {
    return "Fun.Finally_raised: " + to_string3(exn._1);
  }
});
...
// node_modules/melange/domain.js
var atomic_lock = {
  contents: false
};
function atomically(f2) {
...
function new_key(split_from_parent, init3) {
  const key = {
    table: void 0,
    split_from_parent,
    init: init3
...
// node_modules/melange.js/caml_md5.js
...
// node_modules/melange/in_channel.js
var seek = LargeFile.seek_in;
var pos = LargeFile.pos_in;
var length2 = LargeFile.in_channel_length;

// node_modules/melange/digest.js
function string$3(str) {
  return caml_md5_string(str, 0, str.length);
}

// node_modules/melange/int32.js
var unsigned_to_int;
if (word_size !== 32) {
  if (word_size !== 64) {
    throw new MelangeError("Assert_failure", {
...
// node_modules/melange/int64.js
var max_int5 = of_int32(max_int2);
var max_int$1 = max_int;

// node_modules/melange/random.js
...
function full_init(s, seed) {
  const combine = function(accu2, x) {
    return string$3(accu2 + caml_format_int("%d", x));
  };
  ...
  let accu = "x";
  for (let i$1 = 0, i_finish = 54 + max(55, l) | 0; i$1 <= i_finish; ++i$1) {
    const j = i$1 % 55;
    const k = i$1 % l;
...
function bits(s) {
  s.idx = (s.idx + 1 | 0) % 55;
  const curval = get(s.st, s.idx);
  const newval = get(s.st, (s.idx + 24 | 0) % 55) + (curval ^ curval >>> 25 & 31) | 0;
...
function int64(s, bound) {
  ...
  while (true) {
    const b1 = of_int32(bits(s));
    const b2 = lsl_(of_int32(bits(s)), 30);
    const b3 = lsl_(of_int32(bits(s) & 7), 60);
    const r = or_(b1, or_(b2, b3));
    const v = mod_(r, bound);
    if (!i64_gt(sub(r, v), add(sub(max_int$1, bound), one))) {
      return v;
    }
    continue;
  }
...
function bits64(s) {
  const b1 = lsr_(of_int32(bits(s)), 9);
  const b2 = lsr_(of_int32(bits(s)), 9);
  const b3 = lsr_(of_int32(bits(s)), 8);
  return or_(b1, or_(lsl_(b2, 21), lsl_(b3, 42)));
}
var State = {
  make: make5,
  make_self_init,
...
// node_modules/melange/hashtbl.js
var randomized = make3(false);
var prng_key = DLS.new_key(void 0, State.make_self_init);
...
function create3(randomOpt, initial_size) {
  const random = randomOpt !== void 0 ? randomOpt : get4(randomized);
  const s = power_2_above(16, initial_size);
  const seed = random ? State.bits(DLS.get(prng_key)) : 0;
  return {
    size: 0,
    data: make(
      s,
...
// node_modules/solid-ml-browser/hydration.js
function create_context(param) {
  return {
    is_hydrating: false,
    text_nodes: create3(void 0, 16)
  };
}
var current_context = {
  contents: create_context()
};
...
function Make(B) {
  ...
  const mark_downstream = function(node) {
    ...
    for (let i2 = 0, i_finish = node.memo_observers_len; i2 < i_finish; ++i2) {
      const o = get(observers, i2);
      if (o.state === /* Clean */
      0) {
        o.state = /* Pending */
        2;
  ...
  const write_signal = function(signal, value) {
    ...
    _2(run_updates_ref.contents, (function(param) {
      for (let i2 = 0, i_finish = signal.observers_len; i2 < i_finish; ++i2) {
        const o = get(signal.observers, i2);
        if (o.state === /* Clean */
        0) {
          if (o.pure) {
            rt.updates = {
  ...
  const clean_node = function(node) {
    for (let i2 = 0, i_finish = node.sources_len; i2 < i_finish; ++i2) {
      const source_obj = get(node.sources, i2);
      const slot = get(node.source_slots, i2);
      const kind = get(node.source_kinds, i2);
      if (kind === /* Signal_source */
      0) {
        if (source_obj.observers_len > 0) {
          const last_idx = source_obj.observers_len - 1 | 0;
          if (slot < last_idx) {
            const last_observer = get(source_obj.observers, last_idx);
            const last_slot = get(source_obj.observer_slots, last_idx);
      ...
      (function(_prev) {
        ...
        if (m2 !== void 0) {
          if (m2.has_cached && !_2(m2.equals, m2.cached, new_val)) {
            ...
            if (observers !== void 0) {
              for (let i2 = 0, i_finish = m2.comp.memo_observers_len; i2 < i_finish; ++i2) {
                const o = get(observers, i2);
                if (o.state === /* Clean */
                0) {
                  o.state = /* Stale */
  ...
  const create_context3 = function($$default) {
  ...
  return {
    ...
    next_context_id,
    create_context: create_context3,
...
function update_signal(s, f2) {
  _2(set_signal, s, _1(f2, _1(peek_signal, s)));
}
function create_root(f2) {
...
var create_context2 = R.create_context;
var use_context = R.use_context;
var provide_context = R.provide_context;

// node_modules/solid-ml-browser/reactive.js
function create4(equals, v) {
  const s = _2(create_signal, equals, v);
  return [
    s,
    (function(v2) {
      _2(set_signal, s, v2);
...
var update = update_signal;
var Effect = {
  create: create_effect,
  create_with_cleanup: create_effect_with_cleanup,
  untrack
};
function create$1(equals, f2) {
  return _2(create_memo, equals, f2);
}
var Memo = {
  create: create$1,
  get: get_memo,
...
// examples/full_ssr_app/client/client.js
function get_element(id) {
  return get_element_by_id($$document, id);
}
function query_selector2(selector) {
  return query_selector($$document, selector);
}
function query_selector_all2(selector) {
  return query_selector_all($$document, selector);
}
function setup_navigation(param) {
  const nav_links = query_selector_all($$document, "nav a.nav-link");
  iter((function(link) {
...
function hydrate_counter(param) {
  const match = get_element_by_id($$document, "counter-value");
  const match$1 = get_element_by_id($$document, "initial-count");
  if (match === void 0) {
    return;
  }
  if (match$1 === void 0) {
  ...
  const counter_el = valFromOption(match);
  const v = get_attribute(valFromOption(match$1), "value");
  let initial;
  if (v !== void 0) {
    try {
      initial = caml_int_of_string(v);
    } catch (exn) {
      initial = 0;
    }
  } else {
  ...
  const match$2 = Signal.create(void 0, initial);
  const set_count = match$2[1];
  const count = match$2[0];
  _1(Effect.create, (function(param2) {
    const value = _1(Signal.get, count);
    set_inner_html(counter_el, String(value));
  }));
  const btn = get_element_by_id($$document, "decrement");
  if (btn !== void 0) {
    add_event_listener(valFromOption(btn), "click", (function(param2) {
      Signal.update(count, (function(n) {
        return n - 1 | 0;
      }));
    }));
  }
  const btn$1 = get_element_by_id($$document, "increment");
  if (btn$1 !== void 0) {
    add_event_listener(valFromOption(btn$1), "click", (function(param2) {
      Signal.update(count, (function(n) {
        return n + 1 | 0;
      }));
    }));
  }
  const btn$2 = get_element_by_id($$document, "reset");
  if (btn$2 !== void 0) {
    add_event_listener(valFromOption(btn$2), "click", (function(param2) {
      _1(set_count, initial);
    }));
  }
  log("Counter hydrated!");
}
function extract_todo_id(el) {
  const id_str = get_attribute(el, "id");
  if (id_str === void 0) {
    return;
  }
  if (!(id_str.length > 5 && sub4(id_str, 0, 5) === "todo-")) {
    return;
  }
  try {
    return caml_int_of_string(sub4(id_str, 5, id_str.length - 5 | 0));
  } catch (exn) {
    return;
  }
}
function hydrate_todos(param) {
  const list_el = get_element_by_id($$document, "todo-list");
  if (list_el === void 0) {
    return;
  }
  const items = query_selector_all_within(valFromOption(list_el), "li.todo-item");
  const initial_todos = filter_map((function(item) {
    const id = extract_todo_id(item);
    if (id === void 0) {
      return;
    }
    const completed = has_class(item, "completed");
    const span = query_selector_within(item, "span");
    const text = span !== void 0 ? get_inner_text(valFromOption(span)) : "";
    return {
      id,
      text,
      completed
  ...
  const match = Signal.create(void 0, initial_todos);
  const todos = match[0];
  const incomplete_count = Memo.create(void 0, (function(param2) {
    return length(filter((function(t) {
      return !t.completed;
    }), _1(Signal.get, todos)));
  }));
  const status_el = query_selector($$document, ".status");
  if (status_el !== void 0) {
    const status_el$1 = valFromOption(status_el);
    _1(Effect.create, (function(param2) {
      const count = _1(Memo.get, incomplete_count);
      set_inner_html(status_el$1, String(count) + " items remaining");
    }));
  }
  iter((function(item) {
    const id = extract_todo_id(item);
    ...
    const checkbox = query_selector_within(item, "input[type='checkbox']");
    if (checkbox !== void 0) {
      return add_event_listener(valFromOption(checkbox), "change", (function(param2) {
        Signal.update(todos, (function(ts) {
        ...
        const is_completed = exists((function(t) {
          if (t.id === id) {
            return t.completed;
          } else {
            return false;
        ...
        if (is_completed) {
          return add_class(item, "completed");
        } else {
          return remove_class(item, "completed");
        }
      }));
    }
  }), items);
...
function show_hydration_status(param) {
  const el = get_element_by_id($$document, "hydration-status");
  if (el !== void 0) {
    return add_class(valFromOption(el), "active");
  }
}
