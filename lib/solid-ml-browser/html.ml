...
let reactive_text signal =
  ...
  let initial = string_of_int (Reactive_core.peek_signal signal) in
  let txt = get_or_create_text_node key initial in
  Reactive_core.create_render_effect (fun () ->
...
let reactive_text_of fmt signal =
  let key = Hydration.next_hydration_key () in
  let initial = fmt (Reactive_core.peek_signal signal) in
  let txt = get_or_create_text_node key initial in
  Reactive_core.create_render_effect (fun () ->
...
let reactive_text_string signal =
  let key = Hydration.next_hydration_key () in
  let initial = Reactive_core.peek_signal signal in
  let txt = get_or_create_text_node key initial in
  Reactive_core.create_render_effect (fun () ->
